...
public class level_control : MonoBehaviour
{
    ...
    private void ClearNextBlock(ref CreationInfo block)//구멍을 뚫어주는 함수
    {
    ...
    private void UpdateLevel(ref CreationInfo current, CreationInfo previous)
    {
        switch (previous.block_type)
        {
            case Block.TYPE.FLOOR:
                ...
                break;
            case Block.TYPE.HOLE://이전 타입이 HOLE이였을 경우
                current.block_type = Block.TYPE.FLOOR;              
                    if (time > Time.deltaTime * 15)                   
                        current.max_count = Random.Range(5, 10);                        
                break;
        }
    }
